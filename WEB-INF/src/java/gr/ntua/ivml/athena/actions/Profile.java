	...
	//private static final long serialVersionUID = 1L;
			   ...
			   String result="";
			    if(getUaction()==null || getUaction().equalsIgnoreCase("edituser")){
				  // current_user=DB.getUserDAO().findById(user.getDbID(), false );;
					...
					if(current_user.getAthenaRole().equalsIgnoreCase("superuser")){
						...
						if(user.getAthenaRole().equalsIgnoreCase("admin") && user.getOrganization().getPrimaryContact()==user){
							addActionError("Error while changing the user's organization. You are the primary contact for the existing organisation.");
							DB.getSession().evict(current_user);
							
				    		return ERROR;
						}
						else if(og!=null && user.getAthenaRole().equalsIgnoreCase("admin") && og.getAdmincount()<=1){
				        ...
				        String text="A user with login:"+ user.getLogin()
				                    ...
				                    "please use the administration page from the Athena webtool to set them.";
				        String mail_to="";
				        ...
				        result=ms.sendToMany(ms.adminmail, "Athena - new user registration", text, mail_to, current_user.getEmail());
	  ...
	  @Action("Profile_input")
		...
		public String input() throws Exception {
	       ...
	       else if(!user.getAthenaRole().equalsIgnoreCase("superuser") && user.getOrganization()==null){
		 ...
		 public void validateUser(){
			...
			if(!user.getAthenaRole().equalsIgnoreCase("superuser") && (getOrgid()==null || getOrgid().equalsIgnoreCase("0"))){
