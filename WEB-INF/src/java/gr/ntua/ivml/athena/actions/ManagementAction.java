	...
	//private static final long serialVersionUID = 1L;
		   ...
		   try{
			     if(getUaction().equalsIgnoreCase("showuser") || getUaction().equalsIgnoreCase("edituser")){
		    	  ...
		    	  seluser=DB.getUserDAO().findById(Long.parseLong(getId()), false);
					...
					if(seluser.getAthenaRole().equalsIgnoreCase("superuser")){
			   	...
			   	if(!getFieldErrors().isEmpty()){
			    	...
			    	if( getPassword()!= null && getPassword().length()>0) {
						...
						if(getNotice()==true){
							if(seluser.getDbID()==null){
								...
								emailtext="Your account password for the Athena system has been changed to "+
											getPassword()+
											".<BR><BR>If you have any questions about your account please contact "+user.getEmail();
			    	...
			    	if(seluser.getDbID()!=null && emailtext.length()==0 && getNotice()==true){
						...
						emailtext+="<BR>Your Athena account was updated by an administrator of your organization."+
						"<BR><BR>If you have any questions about your account please contact "+user.getEmail();
					}
					if(emailtext.length()>0 && getNotice()==true){
						//sending email
						MailSender ms=new MailSender();
						String result=ms.send(ms.adminmail, "Athena - User account", emailtext, seluser.getEmail());
	  ...
	  public void validateOrg(){
		 ...
		 public void validateUser(){
			...
			if(seluser.getAthenaRole()==null || seluser.getAthenaRole().length()==0){
				addFieldError("seluser.athenaRole","Specify a user role in this project");
			}
			if(!seluser.getAthenaRole().equalsIgnoreCase("superuser") && (getOrgid()==null || getOrgid().equalsIgnoreCase("0"))){
