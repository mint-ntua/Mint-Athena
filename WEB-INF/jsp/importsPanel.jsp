...
form.athform fieldset li {
	background: url("images/formdiv4.gif") left bottom repeat-x;
     }
</style>
...
function toggleSliderImp(e){
	 ...
	 if(slider.style.height=="0px"){
		 var anim = new YAHOO.util.Anim(slider,{height:{to:80}},1,YAHOO.util.Easing.easeOut);
...
<table border="0"  cellpadding="0" cellspacing="0" width="398">
	<tr><td height="10px;">&nbsp;</td></tr>
		<tr><td>
		<table width="398" cellpadding="0" cellspacing="0" height="13" bgcolor="#8CB85C">
			<tr>
				<td height="12" width="13" valign="top" align="left"><img
					src="images/org_top_left.png" width="13" height="12"></td>
				<td height="20"><font style="color: #FFFFFF;">&nbsp;<b>Imports</b>
				</font></td>
...
<div style="padding:5px;background: #FFFFFF;font-size:100%;">
	&nbsp;<b>Organization: </b><s:select theme="simple"  cssStyle="width:150px"  name="filterorg"  list="organizations" listKey="dbID" listValue="name" value="%{o.dbID}"  onChange="javascript:ajaxImportsPanel(0,5,-1,document.getElementById('filterorg').value);"></s:select>
	...
	<a onclick="javascript:ajaxItemPanel(0, 10, <s:property value="orgId"/>, -1,<s:property value="userId"/>);" href="#" class="" title="show all items for organization">
	 ...
	 <%if( (user.getOrganization() == null && user.hasRight(User.SUPER_USER)) || user.hasRight(User.ADMIN)|| (user.hasRight(User.PUBLISH) && user.getOrganization().getDbID()==oid)) {%>
              ...
              <span id="pubstyle">
           			<a href="#" onClick="javascript:this.blur();ajaxPublicationRequest(<s:property value="orgId"/>,<s:property value="userId"/>,<s:property value="startImport"/>)"><img src="images/eutrans.png" style="padding-left:2px;" title="Publish to Europeana"></a>
	 ...
	 <%if( (user.getOrganization() == null && user.hasRight(User.SUPER_USER)) || user.hasRight(User.ADMIN)|| (user.hasRight(User.MODIFY_DATA) && user.getOrganization().getDbID()==oid)) {%>
		...
		<s:if test="pub!=null && pstatus=='OK'">
	     <a href="download.action?orgId=<s:property value="orgId"/>&published=true"> <img src="images/esedown.png" width="33" height="26"  title="Download ESE" style="padding-left:1px;"></a>
...
<s:if test="hasActionMessages()">
		<s:iterator value="actionMessages">
		 ...
		 <div style="padding:5px;font-size:1em;">View all available actions by clicking on an import name</div>
         <ol>
		 <s:iterator id="imp" value="imports">
		<%
		importcount++; %>
						<li style="padding: 0px;padding-bottom: 3px;">
				 
						<table border="0" cellspacing="0" cellpadding="0" width="100%">
                             <tr background="images/sprite.png"> 
			                <td width="15">
			           ...
			           <!-- published can not be deleted before unpublished -->
			            ...
			            <s:elseif test="pub.status=='OK'">
			             <img title="Published import" src="images/esetransp.png" style="padding-left:5px;">
			            ...
			            <%} %>
								...
								<b>
									<s:if test="oai!=''" >
									<a onClick="javascript:toggleSliderImp(<s:property value="dbID"/>);" style="color:#000;cursor:pointer"><s:property value="oai"/></a>
									</s:if>
									<s:else>
										
										   <a onClick="javascript:toggleSliderImp(<s:property value="dbID"/>);" style="color:#000;cursor:pointer"><s:property value="name"/></a>
			                    ...
			                    <s:elseif test="lido==true && pub.status!='NOT DONE'">
			                     <tr><td colspan="4">
			                       <span id="import_pub_<s:property value="dbID"/>" class="yui-skin-sam">
			                             <s:if test="pub.status!='OK' && pub.status!='ERROR' && pub.status!='NOT DONE'">
			                               <div id="s<s:property value="dbID"/>_impclass" class="impclassb">
									      ...
									      <s:else>
									       <div id="s<s:property value="dbID"/>_impclass" class="impclass">
											<div id="sliderimpclass<s:property value="dbID"/>" class="sliderimpclass" style="height: 0px;">
												<div class="scontentimpclass">
					                       				<div style="margin-left:5px;margin-top:10px;">Lido import, no further processing required. This import can be published directly to Europeana.
											 	</div>
											 </div>
											</div>
									      </s:else>
		  	...
		  	<s:if test="imports.size>0">
           			...
           			<table border="0" cellspacing="0" cellpadding="0" width="100%">
						       <tr><td width="20" align="right">
			                    ...
			                    <img src="images/trash_can.png" width="20" height="20" style="vertical-align:middle;">Delete selected</a></td>
		...
		<tr>
			...
			<td><a class="button" id="buttongo" onclick="javascript:this.blur();ajaxImportsPanel(document.getElementById('pageJump_<s:property value="orgId"/>_<s:property value="userId"/>').value*5,5,<s:property value="userId"/>,<s:property value="orgId"/>)"><span>go</span></a></td>
		</tr>
	</table>
