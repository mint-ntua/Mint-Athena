...
<!--
  ...
  Provided for ATHENA project, 2011-01-06. 

  Note: Handling of language variants is as follows:
	For all LIDO descriptive metadata any language variant available is transformed to ESE, each qualified by xml:lang attribute, thereby 
		providing a mechanism to control search and display for different languages.
	For LIDO administrative metadata only the main language is transformed to ESE, referring to the europeana:language attribute.
		...
		<xsl:choose>
			<xsl:when test="lido:rightsWorkWrap/lido:rightsWorkSet/lido:creditLine[string-length(.)&gt;0]">
				<xsl:for-each select="lido:rightsWorkWrap/lido:rightsWorkSet/lido:creditLine[string-length(.)&gt;0]">
					<!-- ignoring alternative names -->
					<dc:rights>
						<xsl:call-template name="langattr-if" />
						<xsl:value-of select="."/>
					</dc:rights>
				</xsl:for-each>
			</xsl:when>
